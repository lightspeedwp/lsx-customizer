...
class Server
{
	...
	protected function getIfModifiedSinceHeader() {
		...
		if ( isset( $_SERVER['HTTP_IF_MODIFIED_SINCE'] ) ) {
			$modifiedSince = sanitize_text_field( wp_unslash( $_SERVER['HTTP_IF_MODIFIED_SINCE'] ) );
	...
	protected function getIfNoneMatchHeader() {
		...
		if ( isset( $_SERVER['HTTP_IF_NONE_MATCH'] ) ) {
			$noneMatch = sanitize_text_field( wp_unslash( $_SERVER['HTTP_IF_NONE_MATCH'] ) );
		}
	...
	public function serve($salt = '')
	{
		...
		if ( 
			...
			$etag =
			$noneMatch = trim( $this->getIfNoneMatchHeader(), '"' );
			...
			if ( $this->needsCompile( $output, $etag ) ) {
				try {
					...
					header( 'Content-type: text/css' ) );
						...
						echo esc_attr( $this->createErrorCSS( $e );
					} else {
							header( $protocol . ' 500 Internal Server Error' );
						header( 'Content-type: text/plain' ) );

						echo esc_attr( 'Parse error: ' . $e->getMessage() . "\n" );
					}
				}
			...
			echo esc_attr( file_get_contents( $output ) );
		...
		$v = Version::VERSION;
		echo esc_attr( "/* INPUT NOT FOUND scss $v */\n" );
	}
